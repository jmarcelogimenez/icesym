...
for irpm = 1:length(rpms)
  ...
  system(["grep '^  0  5' tests/multicyl4TSI/RPM_" num2str(rpm) "/cyl_0.txt > extractedCyl0.txt"]);    
  cyl0 = load(["extractedCyl0.txt"]);
  
  system(["grep '^  0  5' tests/multicyl4TSI/RPM_" num2str(rpm) "/cyl_1.txt > extractedCyl1.txt"]);    
  cyl1 = load(["extractedCyl1.txt"]);
  
  system(["grep '^  0  5' tests/multicyl4TSI/RPM_" num2str(rpm) "/cyl_2.txt > extractedCyl2.txt"]);    
  cyl2 = load(["extractedCyl2.txt"]);
  
  system(["grep '^  0  5' tests/multicyl4TSI/RPM_" num2str(rpm) "/cyl_3.txt > extractedCyl3.txt"]);    
...
figure(3),%clf
 ...
 print -dpdf tests/multicyl4TSI/pow.pdf
